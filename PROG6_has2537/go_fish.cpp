...
int main( )
{
    ...
    bool Turn =false;// when this bool is false its p2's turn, when it is true it is p1's. alternates everyturn

    ofstream outFile;
    outFile.open("gofish_results.txt");
    cout << "Welcome to Go Fish!\nThe contestants are: Jane and Joe\n\n";// have it outing to std output and the file
    ...
    while(isWinner(p1,p2)==0) {//main loop running while no one  has won
        ...
        if (!Turn) { 			// when its p2s turn
            while (!Turn) {			 // p2s turn looop
                ...
                if (p1.rankInHand(cardrequest)) {// if the card requested is in p1's hand
                    ...
                    Turn = true;//sets it to the other players turn
                }
            }
        } else {
            while (Turn) {// main loop for p1
